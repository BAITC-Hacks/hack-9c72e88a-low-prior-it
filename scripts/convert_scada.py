...
def convert(latency_minutes: float):
    ...
    for t in S.TURBINES:
        ...
        out = pd.DataFrame(
            {
                "turbine_id": f"turbine-{t['id']}",
                "valid_time": end,
                "available_at": end + pd.Timedelta(minutes=latency_minutes),
                "wind_speed_ms": hourly.ws.to_numpy(),
                "temperature_c": hourly.temp.to_numpy(),
                "power_normalized": hourly.p.to_numpy(),
            }
        )
        frames.append(out)
        report[str(t["id"])] = {
            ...
            "canonical_hours": len(out),
            "start": end.min().isoformat(),
            "end": end.max().isoformat(),
        }
...
def main():
    ...
    metadata = {
        "source": "Supplied SCADA; coordinate/normalization confirmation from project owner",
        ...
        "latency_status": "explicit caller assumption; not confirmed by case",
