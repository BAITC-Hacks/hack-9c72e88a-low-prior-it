...
def smoke() -> bool:
    ...
    for i, loc in enumerate(locs):
        ...
        print(
            f"[smoke] loc{i + 1}: lat={loc.get('latitude')} lon={loc.get('longitude')} "
...
def run_single_runs(jobs: list[datetime], workers: int) -> list[datetime]:
    ...
    print(
        f"[single-runs] всего запусков {len(jobs)}, в кэше {len(jobs) - len(todo)}, скачать {len(todo)}"
    )
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = {
            ex.submit(om.fetch_single_run, r, lats, lons, S.CACHE_DIR, S.NWP_MODEL): r for r in todo
        }
        for fut in as_completed(futs):
            ...
            if done % 25 == 0 or done == len(todo):
                ...
                print(
                    f"  {done}/{len(todo)}  {el:5.0f} c прошло, ~{eta:5.0f} c осталось, ошибок {len(failed)}"
                )
    return failed
...
def run_previous_runs() -> None:
    ...
    while start <= S.TEST_END:
            ...
            date(start.year + (start.month + 2) // 12, (start.month + 2) % 12 + 1, 1)
            - timedelta(days=1),
            S.TEST_END,
        )
...
def main() -> None:
    ...
    print(
        "Готово."
        if not failed
        else f"Готово, но {len(failed)} запусков не скачались: запустите скрипт ещё раз."
    )
