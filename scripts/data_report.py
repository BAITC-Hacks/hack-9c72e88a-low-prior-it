...
def main():
    ...
    for turbine in S.TURBINES:
        ...
        report["scada"][turbine["name"]] = {
            "rows": len(raw),
            "expected": expected,
            "missing_fraction": 1 - len(raw) / expected,
            "gaps_over_2h": int((gaps > pd.Timedelta(hours=2)).sum()),
            "longest_gap": str(gaps.max()),
            "invalid_values": raw.isna().sum().to_dict(),
            "simple_anomaly_fraction": float(flag_anomalies(raw).mean()),
        }
    for path in sorted(S.WEATHER_DIR.glob("*.csv.gz")):
        data = pd.read_csv(path)
        report["weather"][path.name] = {
            "rows": len(data),
    ...
    wind = (
        archive[(archive["loc"] == 1) & (archive.lead_h < 24)]
        .drop_duplicates("valid_utc")
        .set_index("valid_utc")
        .wind_speed_100m.sort_index()
    )
    ...
    for year in (2024, 2025):
        ...
        for offset in (4, 5, 6, 7, 8):
            ...
            report["wind_timezone_correlations"][str(year)][str(offset)] = float(
                joined.iloc[:, 0].corr(joined.iloc[:, 1])
            )
    report["power_curve_samples"] = {
        str(k): c.predict([3, 5, 7, 9, 11, 13, 20]).tolist() for k, c in curves.items()
    }
    report["notes"] = [
        "Archive coverage does not prove historical availability.",
