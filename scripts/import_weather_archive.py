...
def main():
    ...
    with httpx.Client(timeout=60) as client:
        for (run, loc), rows in selected.groupby(["run_utc", "loc"]):
            points = [
                WeatherPoint(
                    valid_time=r.valid_utc.tz_localize("UTC"),
                    wind_speed_ms=r.wind_speed_100m,
                    temperature_c=r.temperature_2m,
                    wind_direction_deg=r.wind_direction_100m,
                )
                for r in rows.dropna(
                    subset=["wind_speed_100m", "temperature_2m", "wind_direction_100m"]
                ).itertuples()
            ]
            if not points:
                continue
            snapshot = WeatherSnapshot(
                id=f"bundled-ecmwf-{int(loc)}-{run:%Y%m%d%H}",
                turbine_id=f"turbine-{int(loc)}",
                source="open-meteo-single-run",
                weather_model="ecmwf_ifs",
                run_init=run.tz_localize("UTC"),
