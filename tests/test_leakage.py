...
def test_agent_does_not_trust_an_adapter_to_enforce_cutoff():
    ...
    with pytest.raises(ValueError, match="not available"):
        asyncio.run(ForecastAgent(UnsafeProvider(), DemoPowerCurve()).run(request, [Turbine(id="turbine-1", name="One")], lambda _: None))
...
def test_agent_retries_transient_failures():
    ...
    events = []
    request = ForecastRequest(turbine_ids=["turbine-1"], issued_at=ISSUE, weather_source="archive")
    result = asyncio.run(ForecastAgent(provider, DemoPowerCurve()).run(request, [Turbine(id="turbine-1", name="One")], events.append))
...
def test_refresh_creates_new_run_without_overwriting_old_result(client, forecast_request):
    original = snapshot()
    assert client.post("/api/v1/weather/snapshots", json=original.model_dump(mode="json")).status_code == 201
    assert client.post("/api/v1/weather/snapshots", json=original.model_dump(mode="json")).status_code == 409
    ...
    revised = snapshot("archive-b", run_init=ISSUE - timedelta(hours=6), available_at=ISSUE - timedelta(hours=1))
    revised.points[0].wind_speed_ms = 12
    assert client.post("/api/v1/weather/snapshots", json=revised.model_dump(mode="json")).status_code == 201
