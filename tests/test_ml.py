...
def observation(power, valid_time, available_at=None):
    return Observation(
        ...
        available_at=available_at or valid_time,
        wind_speed_ms=8,
        temperature_c=-5,
...
def test_model_cutoff_checked_before_forecast(client, forecast_request):
    upload = client.post(
        ...
        json={
            ...
            "observations": [observation(0.2, ISSUE).model_dump(mode="json")],
        },
    )
    ...
    response = client.post(
        "/api/v1/forecasts",
        json=forecast_request | {"model_id": trained.json()["id"], "turbine_ids": ["turbine-1"]},
    )
...
def test_evaluation_keeps_horizons_and_handles_zero_power():
    ...
    points = [
        ...
        ForecastPoint(
            turbine_id="turbine-1", valid_time=valid, lead_hours=25, power_normalized=0.4
        ),
        ForecastPoint(
            turbine_id="turbine-1",
...
def test_duplicate_observations_rejected(client):
    ...
    response = client.post(
        "/api/v1/datasets", json={"name": "duplicates", "observations": [row, row]}
    )
    assert response.status_code == 422


def test_artificial_training_retains_demo_label(client):
    row = observation(0.2, ISSUE).model_dump(mode="json")
    dataset = client.post(
        "/api/v1/datasets", json={"name": "artificial", "is_demo": True, "observations": [row]}
    ).json()
    trained = client.post(
        "/api/v1/models/train",
        json={"dataset_id": dataset["id"], "trained_through": ISSUE.isoformat()},
    ).json()
    assert dataset["is_demo"] is True
    assert trained["is_demo"] is True
