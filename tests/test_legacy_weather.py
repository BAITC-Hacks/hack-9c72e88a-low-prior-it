...
def test_archive_merges_new_cached_run_even_when_csv_has_more_runs(tmp_path, monkeypatch):
    ...
    pd.DataFrame(
        [
            {
                "run_utc": "2026-01-01 00:00",
                "loc": 1,
                "valid_utc": "2026-01-01 01:00",
                "wind_speed_100m": 1,
                "lead_h": 1,
            },
            {
                "run_utc": "2026-01-02 00:00",
                "loc": 1,
                "valid_utc": "2026-01-02 01:00",
                "wind_speed_100m": 2,
                "lead_h": 1,
            },
        ]
    ...
    (cache / "2026010300.json").write_text(
        json.dumps({"hourly": {"time": ["2026-01-03T01:00"], "wind_speed_100m": [3]}})
    )
    result = weather.load_archive()
    assert len(result) == 3 and result.run_utc.nunique() == 3
    assert (
        weather.load_single_run(
            datetime(2026, 1, 1, tzinfo=UTC), offline=True
        ).wind_speed_100m.iloc[0]
        == 1
    )
