...
from windagent.scada import PowerCurve, farm_hourly, load_all_hourly, load_turbine_10min  # noqa: E402
...
def test_scada_hourly_is_clean(scada):
    ...
    # шкала SCADA = UTC+6
    assert (long["time_local"] - long["time_utc"]).eq(pd.Timedelta(hours=S.SCADA_UTC_OFFSET_H)).all()
...
def test_latest_available_run_respects_publication_delay():
    ...
    issue = datetime(2026, 2, 10, 7, tzinfo=timezone.utc)
    assert om.latest_available_run(issue, 6.0) == datetime(2026, 2, 10, 0, tzinfo=timezone.utc)
    early = datetime(2026, 2, 10, 5, tzinfo=timezone.utc)
    assert om.latest_available_run(early, 6.0) == datetime(2026, 2, 9, 18, tzinfo=timezone.utc)
...
def test_second_model_no_leak():
    ...
    prev = pd.DataFrame({"loc": 1, "valid_utc": valid,
                         "wind_speed_80m_previous_day1": 1.0, "wind_speed_80m_previous_day2": 2.0,
...
def test_scada_time_scale_is_utc_plus_6(scada):
    ...
    long, _, _ = scada
    nwp = weather.load_archive()
    n = nwp[(nwp["loc"] == 1) & (nwp["lead_h"] < 24)].drop_duplicates("valid_utc").set_index("valid_utc")
