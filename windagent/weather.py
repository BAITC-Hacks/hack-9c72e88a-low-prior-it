...
def _compiled_archive(model: str) -> pd.DataFrame | None:
    if model not in _ARCHIVE:
        ...
        _ARCHIVE[model] = (
            pd.read_csv(p, parse_dates=["run_utc", "valid_utc"]) if p else pd.DataFrame()
        )
    df = _ARCHIVE[model]
...
def load_single_run(
    run_utc: datetime, model: str = S.NWP_MODEL, offline: bool = False, refresh: bool = False
) -> pd.DataFrame:
    ...
    locs = om.fetch_single_run(
        run_utc, lats, lons, S.CACHE_DIR, model=model, offline=offline, use_cache=not refresh
    )
...
def load_archive(model: str = S.NWP_MODEL) -> pd.DataFrame:
    ...
    for p in files:
        run = datetime.strptime(p.stem, "%Y%m%d%H").replace(tzinfo=UTC)
    ...
    return (
        ...
        .drop_duplicates(["run_utc", "loc", "valid_utc"], keep="last")
        .sort_values(["run_utc", "loc", "valid_utc"])
...
def second_model_as_of(
    prev: pd.DataFrame,
    issue_utc: pd.Timestamp,
    valid_utc: pd.Series,
    delay_h: float = S.NWP_PUBLISH_DELAY_H,
    loc: int = 1,
) -> pd.DataFrame:
    ...
    ks = sorted(
        {int(c.rsplit("_previous_day", 1)[1]) for c in prev.columns if "_previous_day" in c}
    )
    lead_days = np.ceil(((valid_utc - issue_utc) / pd.Timedelta(hours=1) + delay_h) / 24.0).clip(
        lower=1
    )
    ...
    for v in base:
        ...
        for k in ks:
            ...
            if sel.any():
                vals[sel] = (
                    rows[col]
                    .reindex(list(zip([loc] * sel.sum(), valid_utc[sel], strict=True)))
                    .to_numpy()
                )
        out[v] = vals
