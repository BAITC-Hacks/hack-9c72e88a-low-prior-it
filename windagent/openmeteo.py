...
* Single Runs API - полный прогноз одного конкретного запуска модели
  ...
  в сохранённом CSV есть с 14.03.2024. Это кандидаты: историческую
  доступность исходного оперативного прогноза нужно подтвердить отдельно.
...
def previous_runs_variables(
    base_vars: Iterable[str] = PREV_BASE_VARS, offsets: Iterable[int] = PREV_OFFSETS
) -> list[str]:
    out: list[str] = []
    for v in base_vars:
        for k in offsets:
...
def latest_available_run(
    issue_utc: datetime, delay_hours: float = 6.0, cycle_hours: int = 6
) -> datetime:
