...
export default function AgentPanel({ run }: { run: ForecastRun | null }) {
  ...
  const provider = analysis?.provider === 'openai' ? 'OpenAI' : 'NVIDIA';
  const status = !run ? 'Idle' : run.status === 'succeeded' ? 'Complete' : run.status === 'failed' ? 'Failed' : 'Running';
  const reached = Math.max(-1, ...events.map(event => steps.findIndex(step => step.stage === event.stage)));
  return <section className="panel agent-panel" id="activity" aria-labelledby="agent-title">
    ...
    <div className="agent-steps">{steps.map((step, i) => { const done = run?.status === 'succeeded' || reached > i; const failed = run?.status === 'failed' && reached === i; return <div key={step.stage} className={`step ${done ? 'completed' : failed ? 'failed' : ''}`}><span>{done ? <Icon name="check" size={12} /> : failed ? <Icon name="close" size={12} /> : String(i + 1).padStart(2, '0')}</span>{step.title}</div>; })}</div>
    ...
    {analysis && <div className="agent-analysis" aria-label={`${provider} advisory analysis`}>
      <strong>{provider} analysis · {analysis.status === 'succeeded' ? 'Complete' : 'Unavailable'}</strong>
      <span className="muted small">{analysis.model}</span>
      {analysis.status === 'succeeded' ? <p>{analysis.summary}</p> : <p>Analysis unavailable ({analysis.error_code}). The numerical forecast is still available.</p>}
      <span className="muted small">Advisory text; forecast values are calculated by the prediction model.</span>
    </div>}
