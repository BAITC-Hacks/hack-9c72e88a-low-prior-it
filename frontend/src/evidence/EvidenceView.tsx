...
import { poolMetrics, sameCoverage } from './metrics';
import './evidence.css';
...
export default function EvidenceView({ active = true }: Props) {
  ...
  const selected = comparison.find(model => model.id === 'selected');
  const persistence = comparison.find(model => model.id === 'persistence');
  const comparable = !!selected && !!persistence && sameCoverage(selected.visibleMetrics, persistence.visibleMetrics);
  const reduction = comparable && selected?.summary && persistence?.summary && persistence.summary.mae > 0
    ? (1 - selected.summary.mae / persistence.summary.mae) * 100 : null;
  ...
  return <section className="evidence-workspace" id="evidence" aria-labelledby="evidence-heading">
    ...
    {report && <div className={loading ? 'evidence-content evidence-refreshing' : 'evidence-content'} aria-busy={loading}>
      <section className="evidence-hero" aria-labelledby="evidence-result-title">
        <div className="evidence-hero-copy"><span className="evidence-eyebrow">Chronological benchmark<span className="evidence-provisional"><span className="status-dot" />Provisional</span></span><h3 id="evidence-result-title">A forecast is only as useful<br className="evidence-desktop-break" /> as the evidence behind it.</h3><p>{report.benchmark.name}. The selected model is compared with persistence on matching forecast targets.</p><div className="evidence-scope"><span><Icon name="calendar" size={14} />{shortDate(report.benchmark.first_issue)}–{date(report.benchmark.last_issue)} issue dates</span><span><Icon name="clock" size={14} />{report.benchmark.horizon_hours}-hour horizon</span></div></div>
        <div className="evidence-hero-result" aria-live="polite"><span className="evidence-result-label">Selected model · mean absolute error</span><div className="evidence-primary-number">{selected?.summary ? score(selected.summary.mae) : '—'}<span>p.u.</span></div><div className={`evidence-result-change ${reduction != null && reduction >= 0 ? 'improved' : reduction != null ? 'regressed' : ''}`}><Icon name={reduction != null && reduction >= 0 ? 'chart' : 'info'} size={16} />{reduction == null ? 'Paired comparison unavailable' : `${Math.abs(reduction).toFixed(1)}% ${reduction >= 0 ? 'lower' : 'higher'} MAE than persistence`}</div><p>{selected?.summary ? `${number.format(selected.summary.samples)} scored issue / target pairs` : 'No observations in this selection'}{turbine !== 'all' || horizon !== 'all' ? ' · filtered below' : ' · both turbines, both lead windows'}</p><span className="evidence-unit-note">p.u. = normalized power. Lower error is better.</span></div>
      </section>

      <div className="evidence-caveat"><Icon name="info" size={17} /><p><strong>{report.benchmark.reused_comparison ? 'January is a reused comparison window.' : 'Provisional evaluation.'}</strong> {report.benchmark.reused_comparison ? 'It is not an untouched holdout. ' : ''}These results describe this benchmark model, not the currently selected forecast. No February result or calibrated confidence is claimed.</p></div>
      ...
      <section className="evidence-section" aria-labelledby="evidence-comparison-title">
        ...
        <div className="evidence-comparison-table table-scroll"><table><caption className="sr-only">Chronological model comparison. Errors are in normalized power units; lower values are better.</caption><thead><tr><th scope="col">Model</th><th scope="col">Mean absolute error <span>p.u.</span></th><th scope="col">RMSE <span>p.u.</span></th><th scope="col">Scored pairs</th></tr></thead><tbody>{comparison.map(model => <tr key={model.id} className={model.id === 'selected' ? 'evidence-selected-row' : ''}><th scope="row"><span className="evidence-model-name">{model.label}{model.id === 'selected' && <span className="evidence-selected-tag">Selected</span>}</span><span className="evidence-model-role">{model.id === 'selected' ? 'Chosen development candidate' : model.id === 'previous' ? 'Previous model configuration' : 'Last available observation'}</span></th><td><div className="evidence-error-value"><strong>{model.summary ? score(model.summary.mae) : '—'}</strong><span className="evidence-error-track" aria-hidden="true"><span style={{ width: `${(model.summary?.mae || 0) / maxError * 100}%` }} /></span></div></td><td className="numeric">{model.summary ? score(model.summary.rmse) : '—'}</td><td className="numeric">{model.summary ? number.format(model.summary.samples) : '—'}</td></tr>)}</tbody></table></div>
